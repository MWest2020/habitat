...
- [ ] 1.1 `worker/entrypoint.sh`: na de checkout, als `HABITAT_TASK_REF` gezet is,
      ...
      ontbrekend pad
- [ ] 1.2 `dispatch/job-template.yaml` + `dispatch/dispatch.sh`: `HABITAT_TASK_REF`
...
- [ ] 1.3 Test: run met een geldig taakbestand → context zichtbaar in de
      ...
      ongewijzigd
- [ ] 1.4 Kooi-bewijs: `git diff` op `cage/` is leeg; geen wijziging aan
      NetworkPolicies/RBAC/egress
- [ ] 1.5 Docs: `docs/reference/dispatch.md` — de optionele taak-referentie +
      de pad-begrenzing
